...
const SandraWidget: React.FC<SandraWidgetProps> = ({ onNewMessage }) => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: '¡Bienvenido a GuestsValencia! Soy Sandra, tu concierge de lujo. ¿En qué rincón de Valencia deseas hospedarte hoy?', timestamp: new Date() }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: 'Perdona, la conexión espacial ha fallado. ¿Podrías repetirlo?', timestamp: new Date() }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <div className="fixed bottom-12 right-12 z-[9999]">
      <button 
        ...
        className={`w-24 h-24 rounded-full shadow-4xl flex items-center justify-center transition-all transform hover:scale-110 active:scale-95 border-2 border-white/20 overflow-hidden glossy-crystalline animate-orbital ${isOpen ? 'bg-brand-900' : 'bg-gradient-to-br from-brand-600 to-indigo-800'}`}
      >
        {isOpen ? (
      ...
      {isOpen && (
        <div className="absolute bottom-28 right-0 w-[95vw] sm:w-[450px] h-[750px] max-h-[85vh] liquid-glass rounded-[50px] shadow-5xl flex flex-col overflow-hidden border border-white/15 animate-fade-in origin-bottom-right glossy-crystalline">
          {/* GALAXY HEADER */}
          <div className="p-8 text-white flex justify-between items-center border-b border-white/10 bg-white/5">
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-[10px] text-brand-400 uppercase tracking-[0.3em] font-bold mt-1">Luxury Concierge 24/7</p>
              </div>
            </div>
            <div className="flex gap-3">
               <button className="w-12 h-12 bg-green-600/20 text-green-400 rounded-full border border-green-500/30 flex items-center justify-center hover:bg-green-600/30 transition-all">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 005.455 5.455l.774-1.548a1 1 0 011.06-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" /></svg>
               </button>
            </div>
          </div>
                  ...
                  {m.role === 'model' && (
                    <div className="flex gap-4 mt-6 pt-4 border-t border-white/10">
                      <button 
                        ...
                        className="text-[10px] font-bold text-brand-400 uppercase tracking-widest hover:text-white transition-all flex items-center gap-2"
                      >
          ...
          <div className="p-8 border-t border-white/10">
            <div className="flex gap-4 items-center bg-white/5 rounded-[30px] px-8 py-4 border border-white/15 focus-within:bg-white/10 transition-all">
              <input 
                ...
                className="flex-1 bg-transparent border-none outline-none text-white text-sm py-2 placeholder:text-slate-500 font-light"
              />
              <button 
                onClick={handleSend}
