...
async function updateBuildCommand(serviceId) {
  ...
  if (response.status === 200) {
    ...
    console.log('\n📊 Nueva configuración:');
    console.log(`   - Build Command: ${updatedService?.buildCommand || newBuildCommand}`);
    console.log(`   - Start Command: ${updatedService?.startCommand || '(auto)'}`);
    console.log(`   - Root Directory: ${updatedService?.rootDir || '(raíz)'}`);
    
    // Verificar que se guardó correctamente
    if (updatedService?.buildCommand === newBuildCommand || response.status === 200) {
      console.log('\n✅ Verificación: Build Command configurado correctamente');
    }
...
async function triggerDeploy(serviceId) {
  ...
  // Render requiere un objeto vacío o sin body para algunos endpoints
...
async function main() {
  ...
  // 1. Usar SERVICE_ID conocido o buscar
  if (!SERVICE_ID || SERVICE_ID === null) {
    ...
    if (!SERVICE_ID) {
      ...
      console.log('      node configurar-build-command-render.cjs srv-XXXXXXXXX');
      console.log('   2. O busca el SERVICE_ID en Render Dashboard y actualiza el script');
      process.exit(1);
    }
  } else {
    console.log(`\n📌 Usando SERVICE_ID configurado: ${SERVICE_ID}\n`);
  }
