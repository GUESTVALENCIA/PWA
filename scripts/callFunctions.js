...
const toolMap = {
  ...
  'fetchUrl': async (args) => {
    ...
    const { url } = args;
    if (!url) {
      return { success: false, message: 'Falta parámetro requerido: url' };
    }
  ...
  'readGitHubFile': async (args) => {
    ...
    if (!owner || !repo || !path) {
      return { success: false, message: 'Faltan parámetros requeridos: owner, repo, path' };
    }
  ...
  'executeMCPCommand': async (args) => {
    ...
    const { command } = args;
    if (!command) {
      return { success: false, message: 'Falta parámetro requerido: command' };
    }
  ...
  'readLocalFile': async (args) => {
    ...
    const { filePath } = args;
    if (!filePath) {
      return { success: false, message: 'Falta parámetro requerido: filePath' };
    }
