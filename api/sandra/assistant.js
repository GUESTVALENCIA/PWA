    ...
    try {
      ...
      if (!aiRes.ok) {
        ...
        if (isProduction && !useGroq && process.env.GROQ_API_KEY) {
          ...
          console.warn(' OpenAI falló en producción');
          ...
          console.warn(' Usando Groq (Qwen) como fallback...');
          ...
          console.warn(' Falló, usando Gemini como fallback');
      ...
      if (process.env.GEMINI_API_KEY && !useGemini && error.message.includes('API')) {
        ...
        console.warn(' Error con API, usando Gemini como último recurso:', error.message);
...
async function handleGeminiConversation(req, res, transcription, messages, conversation) {
  ...
  try {
    ...
    const fallbackModels = String(process.env.GEMINI_FALLBACK_MODELS || 'gemini-2.5-flash,gemini-1.5-flash')
      .split(',')
      .map((item) => item.trim())
      .filter(Boolean);
    ...
    return res.status(200).json({
      ...
      model: usedModel || (process.env.GEMINI_MODEL || 'gemini-2.5-flash-lite'),
