...
// Override: PREFERRED_AI_PROVIDER=gemini fuerza Gemini primero en cualquier entorno.
    ...
    if (this.isProduction) {
      ...
      try {
        ...
        console.warn(" GPT-4o falló:", openaiError.message);

        console.warn(" Detalle del error OpenAI:", {
        ...
        console.warn(" Intentando Groq (Qwen) como fallback...");
        ...
        try {
          ...
          console.warn(" Groq (Qwen) falló:", groqError.message);
          ...
          console.warn(" Intentando Groq (DeepSeek) como fallback...");
          ...
          try {
            ...
            console.warn(" Groq (DeepSeek) falló:", deepseekError.message);
            ...
            console.warn(" Usando Gemini como último recurso...");
      ...
      try {
        ...
        console.warn(" Gemini falló, intentando GPT-4o...", error.message);
  ...
  try {
    ...
    if (endpoint === 'api-gateway' || endpoint === '') {
      ...
      if (fullUrl) {
        ...
        console.warn(' [API Gateway] No se pudo determinar endpoint, usando req.url completo');
      ...
      switch (endpoint) {
        ...
        case 'sandra/chat':
          ...
          try {
            ...
            console.log(' [API Gateway] Llamando OpenAI (gpt-4o-mini) para chat...');

            const role = chatBody.role || 'hospitality';
        ...
        case 'sandra/voice-call':

          // Voz conversacional: proxy hacia servidor MCP (Render) para pipeline completo
          if (!parsedBody || (!parsedBody.audio && !parsedBody.transcription)) {
            res.setHeader('Access-Control-Allow-Origin', '*');
            return res.status(400).json({ error: 'Missing audio/transcription in request body' });
          }
          ...
          try {
            ...
            const timezone = parsedBody.timezone || 'Europe/Madrid';
            ...
            const mcpResponse = await fetch(`${mcpServerUrl}/api/conserje/voice-flow`, {
              method: 'POST',
              headers,
              body: JSON.stringify({
                audio: parsedBody.audio || null,
                transcription: parsedBody.transcription || '',
                timezone,
                provider: parsedBody.provider || 'gpt4',
                conversationHistory: Array.isArray(parsedBody.conversationHistory) ? parsedBody.conversationHistory : []
              })
            });

            if (!mcpResponse.ok) {
              const errText = await mcpResponse.text();
              throw new Error(`MCP Error: ${mcpResponse.status} - ${mcpResponse.statusText}. Response: ${errText}`);
            }

            const mcpData = await mcpResponse.json().catch(() => ({}));
            const transcript = mcpData?.flow?.transcript || parsedBody.transcription || '';
            const reply = mcpData?.flow?.response || mcpData?.response || '';
            const audio = mcpData?.flow?.audio || mcpData?.audio || null;

            if (!reply) {
              throw new Error('MCP did not return a valid response');
            }
            ...
            console.error(' [API Gateway] Error en sandra/voice-call:', {
