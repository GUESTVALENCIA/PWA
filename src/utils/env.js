  ...
  if (isBrowser) {
    ...
    if (isVite) {
      ...
      if (viteKeys[provider]) {
        console.warn(`⚠️ getApiKeyFor('${provider}') exponiendo API key en cliente. No recomendado para producción.`);
        return viteKeys[provider];
      }
    }
    
    console.warn(`⚠️ getApiKeyFor('${provider}') no disponible en navegador. Las API keys deben estar en el servidor.`);
    return '';
  }
...
export const connectGalaxyToSandra = (widget, options = {}) => {
  if (!widget) {
    console.warn('⚠️ connectGalaxyToSandra: Widget no proporcionado');
    return;
  }
  ...
  if (typeof widget.onUserMessage === 'function') {
    ...
    console.log('ℹ️ connectGalaxyToSandra: Función asignada como widget.handleUserMessage');
  }
...
export const speakSandraVoice = (text, options = {}) => {
  ...
  if (!text || typeof text !== 'string' || text.trim() === '') {
    console.warn('⚠️ speakSandraVoice: Texto vacío o inválido');
    return;
  }
...
export const pauseSandraVoice = () => {
  ...
  console.log('⏸️ Voz de Sandra pausada');
};

/**
 * ▶️ Reanudar voz de Sandra
 */
export const resumeSandraVoice = () => {
  ...
  console.log('▶️ Voz de Sandra reanudada');
};
