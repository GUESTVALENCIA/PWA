 ...
 */
export function safeGetElement(id, options = {}) {
        ...
        return new Promise((resolve) => {
            ...
            const tryGet = () => {
                ...
                if (el) {
                    ...
                    console.warn(` Elemento no encontrado después de ${maxRetries} intentos: ${id}`);
                    resolve(null);
                }
            };
...
export function getElement(id) {
    try {
        ...
        console.warn(` Error obteniendo elemento ${id}:`, e);
        return null;
    }
}
...
export function safeQuerySelector(selector, context = document) {
    try {
        ...
        console.warn(` Error en querySelector "${selector}":`, e);
        return null;
    }
}
...
export function safeQuerySelectorAll(selector, context = document) {
    try {
        ...
        console.warn(` Error en querySelectorAll "${selector}":`, e);
        return [];
    }
}
 ...
 */
export function withElement(id, callback, defaultValue = null) {
    const element = getElement(id);
    if (element && typeof callback === 'function') {
        try {
            return callback(element);
        } catch (e) {
            console.error(` Error ejecutando callback para elemento ${id}:`, e);
            return defaultValue;
        }
    }
    return defaultValue;
 ...
 * @returns {*} - Resultado de callback o defaultValue
 */
export function safeElementAccess(element, callback, defaultValue = null) {
    if (element && typeof callback === 'function') {
        try {
            return callback(element);
        } catch (e) {
            console.error(' Error accediendo a elemento:', e);
            return defaultValue;
        }
    }
    return defaultValue;
