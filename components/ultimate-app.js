        ...
        return function sync() {
            ...
            window.modelSelector.multiModel = multiModel === 'true';
                ...
                if (attempts < MAX_ATTEMPTS) {
                    ...
                    console.warn(' ModelSelector no disponible después de múltiples intentos');
                }
            }
        };
    })();
...
function setupDragAndDrop() {
    ...
    if (!dropZone) {
        console.warn(' cursor-chat-bar no encontrado, saltando setupDragAndDrop');
        return;
    }
...
function initModelList() {
    ...
    console.warn(' ModelSelector no disponible aún, reintentando...');
    setTimeout(() => {
        if (window.modelSelector && typeof window.modelSelector.renderModelList === 'function') {
            window.modelSelector.renderModelList();
        } else {
...
function toggleAutoMode(enabled) {
    if (enabled) {
        ...
        console.log('ℹ Modo Auto desactivado');
    }
}

function toggleMaxMode(enabled) {
    console.log(`ℹ MAX Mode ${enabled ? 'activado' : 'desactivado'}`);
...
function toggleMultiModel(enabled) {
    ...
    console.log(`ℹ Multi Model ${enabled ? 'activado' : 'desactivado'}`);
...
function showAddModelModal() {
    console.log('ℹ Función showAddModelModal llamada');
...
function checkVisionCapability(provider, model) {
    ...
    if (!modelInfo) {
        console.warn(` Modelo no encontrado: ${provider}:${model}`);
        return false;
    }
    
    // Si el modelo no tiene visión pero hay imágenes, sugerir cambio
    if (!modelInfo.vision && AppState.attachedImages.length > 0) {
        console.warn(` El modelo ${modelInfo.name} no soporta visión, pero hay imágenes adjuntas.`);
...
function toggleAgentMenu() {
    ...
    if (!menu || !button) {
        console.warn(' Elementos del menú Agente no encontrados', {
            menu: !!menu,
            button: !!button,
    ...
    try {
        ...
        if (window.speakWithAvatar && typeof window.speakWithAvatar === 'function') {
            try {
                ...
                console.warn(' Error haciendo hablar al avatar:', avatarError);
    ...
    try {
        ...
        try {
            await Promise.race([
                ...
                new Promise(resolve => setTimeout(() => {
                    if (!deepgramReady) {
                        console.warn(' Timeout esperando conexión Deepgram, continuando...');
                        updateDeepgramReady(true);
                        if (deepgramReadyResolver) {
                            deepgramReadyResolver();
                            deepgramReadyResolver = null;
        ...
        AppState.recordingState.updateInterval = setInterval(() => {
            ...
            if (elapsed >= AppState.recordingState.maxDuration) {
                showToast('⏱ Tiempo máximo alcanzado (20 minutos)', 'warning');
                stopVoiceRecording();
            }
        }, 1000);
...
async function stopVoiceRecording() {
    ...
    try {
        ...
        if (finalTranscript && finalTranscript.length > 0) {
            ...
            showToast(' No se detectó audio', 'warning');
        }
...
function showLoadingState(message = 'Cargando audio...') {
    ...
    if (!inputWrapper) {
        console.warn(' cursor-chat-bar no encontrado para mostrar loading');
        return;
    }
